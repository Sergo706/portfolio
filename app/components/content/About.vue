      ...
      <div class="flex flex-wrap gap-4">
        ...
        >
          <NuxtLink
            ...
            class="group relative flex flex-col items-center justify-center p-6 h-full overflow-hidden"
          >
            <UIcon
              :name="item.icon"
              class="size-9 transition-all duration-300 ease-in-out group-hover:-translate-y-2 group-hover:scale-110"
              :font-controlled="false"
              :alt="item.name + ' logo'"
              :aria-label="item.name + ' logo'"
            />
            <span class="absolute bottom-2 text-xs font-semibold tracking-wider text-[var(--ui-text-toned)] opacity-0 transition-all duration-300 ease-in-out translate-y-4 group-hover:translate-y-0 group-hover:opacity-100">
              {{ item.name }}
            </span>
          </NuxtLink>
        </SpotlightCard>
